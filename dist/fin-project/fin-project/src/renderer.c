...
/* A project's active window in project-relative days: earliest scheduled task
 * start -> latest task end (unscheduled tasks ignored). Returns 1 if the project
 * has any scheduled task, 0 otherwise. This is "when the project is really busy",
 * which can begin well after its start_date if shared members are committed
 * elsewhere (cross-project floor). */
static int project_active_window(const Project *p, int *out_start, int *out_end) {
    int k, mn = 0, mx = 0, have = 0;
    for (k = 0; k < p->task_count; k++) {
        int s = p->tasks[k]->sched_start, e = p->tasks[k]->sched_end;
        if (s < 0) continue;                       /* unscheduled */
        if (!have) { mn = s; mx = e; have = 1; }
        else { if (s < mn) mn = s; if (e > mx) mx = e; }
    }
    *out_start = have ? mn : 0;
    *out_end   = have ? mx : 0;
    return have;
...
void render_portfolio_gantt(const Company *c, int width) {
    ...
    /* Pass 1: absolute timeline bounds from each project's active window */
    for (i = 0; i < c->project_count; i++) {
        Project *p = c->projects[i];
        int a_start, a_end;
        if (!date_is_valid(p->start_date)) continue;
        if (!project_active_window(p, &a_start, &a_end)) continue;
        {
            long sa = date_to_days(p->start_date) + a_start;   /* abs active start */
            long ea = date_to_days(p->start_date) + a_end;     /* abs active end   */
            if (datable_count == 0) { t0 = sa; t1 = ea; }
            else { if (sa < t0) t0 = sa; if (ea > t1) t1 = ea; }
            datable_count++;
        }
    }
    ...
    /* One row per project: a bar spanning its active window on the company timeline */
    for (i = 0; i < c->project_count; i++) {
        Project *p = c->projects[i];
        int a_start, a_end;
        int scheduled = project_active_window(p, &a_start, &a_end);

        printf("%-24.24s  %5dd  |", p->name, a_end - a_start);

        if (!date_is_valid(p->start_date) || !scheduled) {
            printf("%s(%s)%s", C_DIM,
                   date_is_valid(p->start_date) ? "not scheduled" : "no start date", C_RESET);
        } else {
            long sa = date_to_days(p->start_date) + a_start;
            long ea = date_to_days(p->start_date) + a_end;
