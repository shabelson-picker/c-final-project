...
/* Prompt for a schedule strategy + assignment policy (shared by the per-project
 * and the company-wide Generate). Remembers the strategy in g_last_strategy and
 * sets the assignment policy. Returns 1 with *out_s on success, 0 if cancelled. */
static int prompt_schedule_params(ScheduleStrategy *out_s) {
    int strat, pol;
    printf("  1. Earliest Deadline    2. Risk-Weighted Critical Path    3. Pessimistic (worst-case)\n");
    if (read_nav("  Strategy: ", &strat) != 1 || strat < 1 || strat > 3) return 0;
    *out_s = (strat == 1) ? SCHED_EARLIEST_DEADLINE
           : (strat == 2) ? SCHED_RISK_WEIGHTED_CRITICAL
           : SCHED_PESSIMISTIC;
    g_last_strategy = *out_s;   /* reused by reschedules that don't prompt */
...
/* Collect-then-resolve: for each task left unassigned after a greedy run, notify
 * the user (no qualified member, or low resource) and offer to pull a qualified
 * off-roster company member onto the project. Returns the number of members
 * added; the caller re-runs scheduling if that is > 0. Scheduler stays pure. */
static int resolve_unassigned(Company *c, int project_idx) {
    ...
    for (i = 0; i < p->task_count; i++) {
        ...
        if (mid == -1) {
            cprintf(C_YELLOW, "  [%s] Task [%d] '%s': no company member has the required skills.\n",
                    p->name, t->id, t->title);
            continue;
        }
        {
            ...
            snprintf(msg, sizeof(msg),
                     "  [%s] Task [%d] '%s' has no one qualified on the project. Add %s (%s)?",
                     p->name, t->id, t->title, m->name, m->role);
...
static void act_sched_generate(void *ctx) {
    Project *p = (Project *)ctx;
    ScheduleStrategy s;
    if (!prompt_schedule_params(&s)) return;
    assign_members_greedy(g_company, g_company_project_idx, s);
    schedule_project(p, s);
    render_gantt(p, g_company, GANTT_WIDTH);
    if (resolve_unassigned(g_company, g_company_project_idx) > 0) {
        assign_members_greedy(g_company, g_company_project_idx, s);  /* re-plan with the added member(s) */
        schedule_project(p, s);
        render_gantt(p, g_company, GANTT_WIDTH);
    }
...
/* Company-wide schedule: one unified pass over all projects on a shared member
 * calendar (schedule_company), then surface any unassignable tasks across every
 * project (same notify + offer-to-add cascade as the per-project Generate), and
 * re-coordinate if members were added. Finally show the combined all-task Gantt. */
static void act_company_schedule(void *ctx) {
    Company *c = (Company *)ctx;
    ScheduleStrategy s;
    int i, added = 0;
    if (c->project_count == 0) { cprintf(C_DIM, "  No projects to schedule.\n"); return; }
    if (!prompt_schedule_params(&s)) return;

    schedule_company(c, s);
    for (i = 0; i < c->project_count; i++)
        added += resolve_unassigned(c, i);
    if (added) schedule_company(c, s);   /* re-plan with the added member(s) */

    company_save(c);
    cprintf(C_GREEN, "  Company schedule generated for %d project(s).\n", c->project_count);
    render_company_gantt(c, GANTT_WIDTH);
}
...
static const MenuItem COMPANY_MENU[] = {
    ...
    { "Schedule all",   PRIV_SCHEDULE,       act_company_schedule },
