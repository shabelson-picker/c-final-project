...
- 2026-05-31 ~06:10 ISR: Researched online PM platforms (Jira/Asana/Monday/MS Project/GanttProject/Wrike) + their critiques; wrote findings into EXTRAPOLATION.md. Recurring critiques: complexity/clutter, fake "timeline" views, conflicts surfacing too late, weak reporting + resource management. This app already counters the scheduling ones (real CPM + cross-project calendar), so picked the clearest gap: reporting/resource mgmt.
- 2026-05-31 ~06:20 ISR: Resource workload / over-allocation report DONE + TESTED. New reports.c/.h (added to vcxproj). compute_member_workload (pure, unit-testable): per member, gathers assignments as ABSOLUTE intervals across all dated projects -> task count, committed days, span, and an overallocated flag (sorted-interval overlap check in absolute time, so it catches CROSS-project clashes). render_workload_report prints a zebra table with OVERALLOCATED/ok/idle. Company menu option 7 "Workload" (gated VIEW_PORTFOLIO). Verified tests/test_workload.c: Carol (overlapping across 2 projects) -> OVERALLOCATED, 10 days, span 8; Dave (disjoint) -> ok; Eve -> idle. 7 assert suites green; build 0/0.
- 2026-05-31 ~06:30 ISR: UX fix (user spotted it): the workload report showed raw absolute serial day numbers ("20454..20462") which looked like an overflow. Added date_from_days (civil-from-days, inverse of date_to_days) to project.c/.h and changed the report to show the active window as a real date + duration ("2026-01-01 (+8d)"). Added a date round-trip assertion to test_workload. Build 0/0; suites green.
- 2026-05-31 ~06:40 ISR: ASCII art (user request). print_banner() in main.c - a "PROJECT MAYHEM" figlet banner + subtitle + a colored mini-Gantt motif strip (green/yellow/dim = optimistic/expected/overrun), shown at startup (replaced the plain === title). goodbye() now prints a small boxed Fight-Club quote. ASCII-only (escaped backslashes; no Unicode -> no C4819). Verified visually. Build 0/0.
- 2026-05-31 ~07:00 ISR: Executive HTML report + modern restyle (user requests) DONE + VERIFIED.
  * export_executive_report_html (report_exporter.c/.h): company-wide report - hero, KPI cards (projects/members/tasks/% done/overallocated), Portfolio table (per project: start, tasks, progress bar, duration), and an HR section = the resource workload table (role, tasks, committed days, active window as a date, utilization bar, OVERALLOCATED/ok/idle pill). Reuses compute_member_workload + date_from_days. Company menu option 8 "Exec report" (gated PRIV_REPORT).
  * Modern restyle: shared write_modern_head() with a contemporary stylesheet (CSS variables, gradient hero, cards, KPI tiles, status pills, progress bars, dark-themed Gantt <pre>, responsive). Both the per-project report and the exec report now use it.
  * Build gotcha: literal % in CSS passed through fprintf -> C4476; escaped all (100%%). 
  * Verified tests/test_exec.c: generated HTML inspected - correct structure, 66% done, Carol OVERALLOCATED pill (11 committed days, +9d), Borealis 100%. 7 assert suites green; build 0/0.
- 2026-05-31 ~07:30 ISR: Milestones given a MEANINGFUL ROLE (user feedback). milestone_forecast_day (latest sched_end of attached tasks) + milestone_status -> ON_TRACK / AT_RISK (within 2-day buffer) / LATE / NO_DATA (project.c/.h). Surfaced everywhere: text schedule report (colored Milestones section), milestones menu list (deadline/forecast/status), project HTML report (Forecast + Status pill columns), exec report ("Milestones at risk" KPI + cross-project Milestones table). Verified tests/test_milestone.c. 8 assert suites green; build 0/0.
- 2026-05-31 ~08:00 ISR: Enriched SHA_inc demo (user request) + found/fixed a real bug.
  * seed_sha.c rebuilds all 5 projects as granular multi-phase plans (13-16 tasks) with parallel branches, fan-in, varied status/risk, broad shared rosters, and milestones whose deadlines are tuned to the scheduled forecast -> a realistic ON TRACK/AT RISK/LATE mix across the portfolio. clear also drops stale milestones.
  * BUG (real, in app): project_find_cyclic_paths dereferenced project_find_task() with no NULL check -> crash on a dependency edge to an unknown/forward-ref task during load. Fixed (return 0). Committed separately.
  * RED HERRING worth noting: the seed/load harnesses crashed with a truncated pointer (FFFFFFFF...) - cause was the HARNESS missing #include "persistence.h", so company_load/company_save were implicitly int -> 64-bit pointer truncated. The real app includes the right headers and was never affected; my id-index changes were exonerated by bisection (disabled both indices, still crashed). Lesson: always include the declaring header in harnesses.
- 2026-05-31 ~08:30 ISR: Day-by-day SIMULATION SANDBOX (user request) DONE + TESTED. sim.c/.h (in vcxproj). menu_simulate clones the company via a temp bundle (save->temp->load) so the real project is untouched until 'Apply to real' (8). Turn=day: 'Next day' auto-completes tasks whose sched_end passed (sim_advance_day, pure/tested) and marks active ones in-progress; manager events: complete/cancel/add task, employee quits (unassign + re-plan), hire (skills + roster + re-plan), re-plan, apply, exit (discard). Project menu option 6 'Simulate' (gated SCHEDULE). Verified tests/test_sim.c (day-advance completion + status transitions). 9 assert suites green; build 0/0. (v1 note: sandbox temp dir lingers on discard; apply overwrites real.)
